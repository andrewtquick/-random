        ...
        if cog == 'all':
            for cog in cogs:
                ...
                for command in cog_command:
                    embed_pages.append(
                        discord.Embed(title=command.name, description=command.help, colour=discord.Colour(0xF1C40F))
                        .set_thumbnail(url='https://i.imgur.com/ySoSpp6.png')
                        .add_field(name='\u200b', value='\u200b', inline=False)
                        .add_field(name='Format:', value=f'`!{command.name} {command.usage if command.usage is not None else ""}`'))


        msg = await ctx.send(embed=embed_pages[0])
            ...
            if selected_emoji == '\u25c0':
                if selected_page >= 1:
                    ...
                    if selected_page == 0:
                        ...
                        embed_msg = embed_pages[selected_page].set_footer(text=f'Page {selected_page} of {int(len(embed_pages))}')
                    await msg.edit(embed=embed_msg)
            if selected_emoji == '\u25b6':
